...
in
{
  options.programs.murmur = {
    enable = lib.mkEnableOption "Murmur voice dictation app with Wayland auto-paste support";
    ...
    users = lib.mkOption {
      ...
      description = ''
        ...
        ydotool group (to reach the ydotoold socket) and the uinput group (Murmur's
  ...
  config = lib.mkIf cfg.enable {
    ...
    # do this, and Murmur's linux-fast-paste --uinput needs /dev/uinput directly.
