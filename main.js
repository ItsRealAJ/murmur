...
const BASE_WINDOWS_APP_ID = "com.murmur.app";
...
function resolveAppChannel() {
  const rawChannel = (process.env.MURMUR_CHANNEL || process.env.VITE_MURMUR_CHANNEL || "")
    .trim()
    .toLowerCase();
...
process.env.MURMUR_CHANNEL = APP_CHANNEL;
...
function configureChannelUserDataPath() {
  ...
  const isolatedPath = path.join(app.getPath("appData"), `Murmur-${APP_CHANNEL}`);
...
if (process.platform === "darwin" && app.getName() !== "Murmur") {
  app.setName("Murmur");
}
...
async function startApp() {
  ...
  if (process.platform === "win32" || process.platform === "linux") {
    ...
    // slots just toggle (matching their globalShortcut callbacks and macOS).
