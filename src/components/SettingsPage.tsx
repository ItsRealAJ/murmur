...
const RETENTION_SELECT_CLASS =
  "h-7 rounded border border-border/70 bg-surface-1/80 px-2.5 text-xs font-medium text-foreground shadow-sm backdrop-blur-sm hover:border-border-hover hover:bg-surface-2/70 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary focus:ring-2 focus:ring-ring/30 focus:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50 transition-colors duration-200";
 ...
 */
function SpeechToTextTabs({ renderDictation }: { renderDictation: () => React.ReactNode }) {
  const { t } = useTranslation();
  return (
    <div className="space-y-4">
      <SectionHeader
...
export default function SettingsPage({
  ...
  const coveringWorkspaceNames: string[] = [];
  const isWorkspaceCovered = false;
  const coveringPlanLabel: string | null = null;
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (anyFailed) {
            ...
            window.dispatchEvent(new Event("murmur-models-cleared"));
            showAlertDialog({
                ...
                <SettingsPanelRow>
                  ...
                  >
                    <select
                      ...
                      className="h-7 rounded border border-border/70 bg-surface-1/80 px-2.5 text-xs font-medium text-foreground shadow-sm backdrop-blur-sm hover:border-border-hover hover:bg-surface-2/70 focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary focus:ring-2 focus:ring-ring/30 focus:ring-offset-1 transition-colors duration-200"
                    >
                      ...
                      steps: [
                        ...
                        },
                        {
                          ...
                          desc: t("settingsPage.general.waylandPaste.guide.group.step2Desc", {
                            defaultValue:
                              "Group changes only take effect after a new login session. Log out of your desktop and log back in, then reopen Murmur.",
                          }),
                        },
                      ],
                    },
