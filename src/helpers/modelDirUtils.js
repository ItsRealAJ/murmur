...
function getAsciiSafeFallbackRoot() {
  const candidates = [
    path.join(process.env.ProgramData || "C:\\ProgramData", "Murmur", "cache"),
    path.join(process.env.SystemDrive || "C:", "Murmur", "cache"),
  ];
...
function getPreferredCacheRoot(homeCache) {
  if (process.env.MURMUR_CACHE_ROOT) {
    return process.env.MURMUR_CACHE_ROOT;
  }

  if (process.platform === "win32") {
    const redirectedProfile = process.env.USERPROFILE;
    if (redirectedProfile && path.isAbsolute(redirectedProfile)) {
      return path.join(redirectedProfile, ".cache", "murmur");
    }
  }
  ...
  if (process.platform === "linux") {
    ...
    if (xdgCacheHome && path.isAbsolute(xdgCacheHome)) {
      return path.join(xdgCacheHome, "murmur");
    }
  }
...
function getCacheRoot() {
  ...
  const homeCache = path.join(homeDir, ".cache", "murmur");
